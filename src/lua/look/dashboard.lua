...
local max_height = #header + 8 + (2 * #buttons) -- 8 == top bottom extra spaces + message
...
local function open()
  ...
  for i = start + #header + #message - 1, start + #header + #buttons * 2 + 1, 2 do
    vim.api.nvim_buf_add_highlight(buf, ThVimDashHl, "ThVimButtonsHl", i, horiz_pad_index + 40,
      horiz_pad_index + 42 + string.len(buttons[1][2]))
    vim.api.nvim_buf_add_highlight(buf, ThVimDashHl, "DashButtonsSpl", i, horiz_pad_index + 3, horiz_pad_index + 7)
  end
...
-- Function adpoted from: https://github.com/chadcat7/kodo/blob/main/lua/themes/hl.lua
